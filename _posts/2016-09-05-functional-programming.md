...
当然了本篇不会教会你任何编程技巧或者函数式编程语言, 想要了解这一块的
...
如果我们限定x为自然熟,那么`f(x)`代表接受一个自然数x,返回其后继自然熟的函数。

那么怎么用数学函数描述1到n的和呢? 我们知道有通项公式: `sum(n) = n * (n + 1) / 2`,
但我们没有高斯这么聪明, 只能一个个地累加,怎么办呢? 嗯至少上面的100和101问题给了我们
一个思路, 我们可以定义一个函数:
...
我们来考虑这个情况: 函数`num_to_str`能将数字转变为字符串, 另一个函数`str_sqrt`
能计算字符串代表的数字的平方根。如果前一个函数的返回值被当作后一个函数的输入值, 会
出线什么情况呢? 数字先被转变成了字符串, 又被转变回数字以计算平方根。

如何解决这个问题呢? 我们可以从以下角度下手:

    # 定义类型NumStr
    type NumStr = NumStr Number
    num_to_str(num) = NumStr num
    
    # 定义str_sqrt
    str_sqrt(NumStr num) = sqrt(num)
    str_sqrt(str) = sqrt(str_to_num(str))

首先定义一个记录类型叫NumStr, 它是num_to_str的返回值, num_to_str不进行真正的计算,
而是把数字封装成一个记录。而str_sqrt函数接受NumStr类型的输入, 直接从中拿到记录的
数字, 不再进行字符串序列化/反序列化的过程。

### 生成器与迭代

延迟计算的好处不仅于此, 它甚至可以用来解决一些更棘手的问题。

假如我们有一个算法(类似斐波那契算法), 它会生成一系列的数字, 而另一个函数会从中尝试
...
考虑这个问题: 如果没有告诉你这段程序想干嘛, 你能一眼看出来他是干什么的么?
